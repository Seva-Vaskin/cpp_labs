...
* Если файл реализации (`.c`/`.cpp`) что-то предоставляет другим единицам трансляции, это должно
  иметь объявление в соотсвествующем ему заголовочном файле (`.h`/`.hpp`).
...
* Код должен соблюдать базовые требования к стилю:
  ...
  * Пробелы и расположения управляющих конструкций должны соотвествовать популярным стилям.
...
`Draft: ` и не назначить проверяющего, такой *Merge Request* можно создать заранее (не забудьте к
моменту сдачи поправить!), чтобы задать вопросы или посмотреть на результат автоматической проверки
(*её пока нет*).
