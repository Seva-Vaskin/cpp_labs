...
LONG get_padding_size(LONG row_length) {
    return (PADDING_ALIGNMENT - row_length % PADDING_ALIGNMENT) % PADDING_ALIGNMENT;
}

LONG get_row_length_with_padding(LONG row_length) {
    return row_length + get_padding_size(row_length);
}
...
void load_bmp(BMP *bmp, FILE *stream) {
    ...
    // data reading
    bmp->flat_data = (PIXEL*)malloc(bmp->bit_map_info.bi_size_image * sizeof(PIXEL));
...
BMP* create_bmp(BMP* header_src, int w, int h) {
    ...
    *result = *header_src;
    DWORD image_size = sizeof(PIXEL) * h * get_row_length_with_padding(w);
